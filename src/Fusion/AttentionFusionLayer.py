...
def AttentionFusionLayerFunc3(original_rgb_features, fv_lidar_features, train_fusion_fv_lidar, original_lidar_feats, scope, kernel_lidar=5, kernel_rgb=5, stride_lidar=3, stride_rgb=3, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        original_lidar_feats = conv(original_lidar_feats, 64, kernel=1, stride=1, scope='final_conv_between_modalities_lidar', reuse=reuse)
        original_lidar_feats = batch_norm(original_lidar_feats, scope='final_bn_between_modalities_lidar')
        original_lidar_feats = relu(original_lidar_feats)

        original_rgb_features = tf.concat([rt2, original_rgb_features], axis=-1)
        original_rgb_features = conv(original_rgb_features, 64, kernel=1, stride=1, scope='final_conv_between_modalities_rgb', reuse=reuse)
        original_rgb_features = batch_norm(original_rgb_features, scope='final_bn_between_modalities_rgb')
        original_rgb_features = relu(original_rgb_features)
        

    return original_lidar_feats, original_rgb_features
