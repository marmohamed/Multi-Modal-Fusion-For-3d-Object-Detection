...
def focal_loss(prediction_tensor, target_tensor, weights=None, alpha=0.25, gamma=2):
    ...
    per_entry_cross_ent = - 100 * alpha * (pos_p_sub ** gamma) * tf.log(tf.clip_by_value(sigmoid_p, 1e-8, 1.0)) \
                          - (1 - alpha) * (neg_p_sub ** gamma) * tf.log(tf.clip_by_value(1.0 - sigmoid_p, 1e-8, 1.0))

    # true_count_theta = tf.math.count_nonzero(target_tensor[:, :, :, :], dtype=tf.float32)
...
# import tensorflow as tf
# from loss.utils_vaidation import check_bool, check_float
